...
use esp_hal::i2c::master::{self, I2c, Blocking};
...
// Create static storage for shared resources that need 'static lifetime
static mut I2C_STORAGE: Option<RefCell<I2c<'static, Blocking>>> = None;
static mut DIR_PIN_STORAGE: Option<Output<'static>> = None;
static mut STEP_PIN_STORAGE: Option<Output<'static>> = None;
...
async fn main(spawner: Spawner) {
    ...
    // Store I2C in static storage
    unsafe {
        I2C_STORAGE = Some(RefCell::new(i2c));
        
        // Create I2C wrapper with static reference
        let i2c_wrapper = I2cWrapper::new(I2C_STORAGE.as_ref().unwrap());
        ...
        // Initialize motor pins and store them in static storage
        DIR_PIN_STORAGE = Some(Output::new(dir_pin, Level::Low, OutputConfig::default()));
        STEP_PIN_STORAGE = Some(Output::new(step_pin, Level::Low, OutputConfig::default()));
        
        // Create motor driver with wrapped static pins
        let dir_pin_wrapped = OutputWrapper::new(DIR_PIN_STORAGE.as_mut().unwrap());
        let step_pin_wrapped = OutputWrapper::new(STEP_PIN_STORAGE.as_mut().unwrap());
        ...
        // Spawn both tasks
        spawner.spawn(imu_reading_task(imu)).unwrap();
        spawner.spawn(motor_control_task(motor)).unwrap();
...
async fn imu_reading_task(mut imu: FreeSixIMU<I2cWrapper<'static, I2c<'static, Blocking>>>) {
...
#[embassy_executor::task]
async fn motor_control_task(mut motor: StepperMotor<OutputWrapper<'static>, OutputWrapper<'static>>) {
    ...
    loop {
        ...
        if roll.abs() > DEAD_ZONE_DEGREES {
            ...
            let current_speed = motor.get_current_speed();
            let speed_diff = target_speed - current_speed;
            
            // Apply only a fraction of the speed change for smooth transitions
            ...
            let clamped_speed = new_speed.abs().min(max_speed);
            motor.set_speed(if new_speed > 0.0 { clamped_speed } else { -clamped_speed });
            ...
            if step_count % 500 == 0 && current_speed != 0.0 {
            ...
            if current_speed.abs() > 50.0 {
                ...
                MOTOR_SPEED.store((new_speed * 10.0) as i32, Ordering::Relaxed);
                
                // Still need to step at the reduced speed
                let _ = motor.move_continuous(current_time);
